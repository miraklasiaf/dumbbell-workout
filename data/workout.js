export const workoutData = [
  {
    label: 'Day 1',
    description: 'Day 1: Chest, Shoulders, & Triceps Dumbbell Workout',
    exercises: [
      {
        title: 'Dumbbell Bench Press',
        src: '/static/images/5-day-split/day-1/db-bench-press.png',
        length: '5 sets x 8-10 reps',
        href: 'https://www.muscleandstrength.com/exercises/dumbbell-bench-press.html'
      },
      {
        title: 'Incline Dumbbell Bench Press',
        src: '/static/images/5-day-split/day-1/incline-db-bench-press.png',
        length: '4 sets x 8-10 reps',
        href:
          'https://www.muscleandstrength.com/exercises/incline-dumbbell-bench-press.html'
      },
      {
        title: 'Dumbbell Floor Press',
        src: '/static/images/5-day-split/day-1/db-floor-press.png',
        length: '3 sets x 8-12 reps',
        href: 'https://www.muscleandstrength.com/exercises/dumbbell-floor-press.html'
      },
      {
        title: 'Standing Dumbbell Press',
        src: '/static/images/5-day-split/day-1/standing-db-press.png',
        length: '4 sets x 8-10 reps',
        href: '#'
      },
      {
        title: 'Dumbbell Lateral Press',
        src: '/static/images/5-day-split/day-1/db-lateral-raise.png',
        length: '3 sets x 8-12 reps',
        href: 'https://www.muscleandstrength.com/exercises/dumbbell-lateral-raise.html'
      },
      {
        title: 'Dumbbell Tricep Kickbacks',
        src: '/static/images/5-day-split/day-1/db-tricep-kickback.png',
        length: '3 sets x 8-12 reps',
        href:
          'https://www.muscleandstrength.com/exercises/bent-over-dumbbell-kickback.html'
      }
    ]
  },
  {
    label: 'Day 2',
    description: 'Day 2: Legs & Core Dumbbell Workout',
    exercises: [
      {
        title: 'Dumbbell Goblet Squat',
        src: '/static/images/5-day-split/day-2/db-goblet-squat.png',
        length: '4 sets x 8-10 reps',
        href: 'https://www.muscleandstrength.com/exercises/dumbbell-goblet-squat'
      },
      {
        title: 'Dumbbell Stiff Leg Deadlift',
        src: '/static/images/5-day-split/day-2/db-stiff-leg.png',
        length: '4 sets x 8-10 reps',
        href:
          'https://www.muscleandstrength.com/exercises/dumbbell-stiff-leg-deadlift.html'
      },
      {
        title: 'Dumbbell Reverse Lunge',
        src: '/static/images/5-day-split/day-2/db-reverse-lunge.png',
        length: '4 sets x 8-10 reps Each',
        href: 'https://www.muscleandstrength.com/exercises/dumbbell-rear-lunge.html'
      },
      {
        title: 'Dumbbell Frog Squat',
        src: '/static/images/5-day-split/day-2/db-frog-squat.png',
        length: '3 sets x 8-12 reps',
        href: 'https://www.muscleandstrength.com/exercises/frog-squat.html'
      },
      {
        title: 'Dumbbell Calf Raise',
        src: '/static/images/5-day-split/day-2/db-calf-raise.png',
        length: '4 sets x 20 reps',
        href:
          'https://www.muscleandstrength.com/exercises/seated-dumbbell-calf-raise.html'
      },
      {
        title: 'Weighted Crunch',
        src: '/static/images/5-day-split/day-2/weighted-crunch.png',
        length: '3 sets x 20 reps',
        href: 'https://www.muscleandstrength.com/exercises/weighted-crunch.html'
      },
      {
        title: 'Side Planks',
        src: '/static/images/5-day-split/day-2/side-planks.png',
        length: '3 sets x 20 secs Each',
        href: 'https://www.muscleandstrength.com/exercises/side-hover.html'
      }
    ]
  },
  {
    label: 'Day 3',
    description: 'Day 3: Back & Biceps Dumbbell Workout',
    exercises: [
      {
        title: 'Dumbbell Bent Over Row',
        src: '/static/images/5-day-split/day-3/db-bent-over-row.png',
        length: '4 sets x 8-12 reps',
        href: 'https://www.muscleandstrength.com/exercises/bent-over-dumbbell-row.html'
      },
      {
        title: 'Tripod Dumbbell Row',
        src: '/static/images/5-day-split/day-3/tripod-db-row.png',
        length: '4 sets x 8-12 reps Each',
        href: 'https://www.muscleandstrength.com/exercises/tripod-dumbbell-row'
      },
      {
        title: 'Dumbbell Pullover',
        src: '/static/images/5-day-split/day-3/db-pullover.png',
        length: '3 sets x 8-12 reps',
        href: 'https://www.muscleandstrength.com/exercises/dumbbell-pullover.html'
      },
      {
        title: 'Reverse Grip Dumbbell Row',
        src: '/static/images/5-day-split/day-3/reverse-grip-db-row.png',
        length: '4 sets x 8-12 reps',
        href:
          'https://www.muscleandstrength.com/exercises/reverse-grip-bent-over-dumbbell-row.html'
      },
      {
        title: 'Dumbbell Bicep Curl',
        src: '/static/images/5-day-split/day-3/db-bicep-curl.png',
        length: '3 sets x 10-15 reps',
        href: 'https://www.muscleandstrength.com/exercises/standing-dumbbell-curl.html'
      },
      {
        title: 'Dumbbell Hammer Curl',
        src: '/static/images/5-day-split/day-3/db-hammer-curl.png',
        length: '3 sets x 10-15 reps',
        href: 'https://www.muscleandstrength.com/exercises/standing-hammer-curl.html'
      }
    ]
  },
  {
    label: 'Day 4',
    description: 'Day 4: Legs & Core Dumbbell Workout',
    exercises: [
      {
        title: 'Dumbbell Squat',
        src: '/static/images/5-day-split/day-4/db-squat.png',
        length: '4 sets x 8-10 reps',
        href: 'https://www.muscleandstrength.com/exercises/dumbbell-squat.html'
      },
      {
        title: 'Dumbbell Deadlift',
        src: '/static/images/5-day-split/day-4/db-deadlift.png',
        length: '4 sets x 8-10 reps',
        href: 'https://www.muscleandstrength.com/exercises/dumbbell-deadlift.html'
      },
      {
        title: 'Dumbbell Split Squat',
        src: '/static/images/5-day-split/day-4/db-split-squat.png',
        length: '3 sets x 8-12 reps each',
        href: 'https://www.muscleandstrength.com/exercises/dumbbell-split-squat'
      },
      {
        title: 'Dumbbell Hip Thrust',
        src: '/static/images/5-day-split/day-4/db-hip-thrust.png',
        length: '4 sets x 10-15 reps',
        href:
          'https://www.muscleandstrength.com/exercises/single-leg-hip-thrust-with-dumbbell'
      },
      {
        title: 'Dumbbell Calf Raise',
        src: '/static/images/5-day-split/day-4/db-calf-raise.png',
        length: '4 sets x 20 reps',
        href:
          'https://www.muscleandstrength.com/exercises/seated-dumbbell-calf-raise.html'
      },
      {
        title: 'Dumbbell Side Bends',
        src: '/static/images/5-day-split/day-4/db-side-bends.png',
        length: '4 sets x 15 reps each',
        href: 'https://www.muscleandstrength.com/exercises/dumbbell-side-bends.html'
      },
      {
        title: 'Plank',
        src: '/static/images/5-day-split/day-4/plank.png',
        length: '3 sets x 20 secs',
        href: 'https://www.muscleandstrength.com/exercises/hover.html'
      }
    ]
  },
  {
    label: 'Day 5',
    description: 'Day 5: Complete Upper Body Dumbbell Workout',
    exercises: [
      {
        title: 'One Arm Dumbbell Rows',
        src: '/static/images/5-day-split/day-5/one-arm-db-rows.png',
        length: '4 sets x 8-10 reps each',
        href: 'https://www.muscleandstrength.com/exercises/one-arm-dumbbell-row.html'
      },
      {
        title: 'Dumbbell Arnold Press',
        src: '/static/images/5-day-split/day-5/db-arnold-press.png',
        length: '4 sets x 8-10 reps',
        href: 'https://www.muscleandstrength.com/exercises/seated-arnold-press.html'
      },
      {
        title: 'Incline Dumbbell Bench Press',
        src: '/static/images/5-day-split/day-5/incline-db-bench-press.png',
        length: '4 sets x 8-12 reps',
        href:
          'https://www.muscleandstrength.com/exercises/incline-dumbbell-bench-press.html'
      },
      {
        title: 'Chest Supported Dumbbell Row',
        src: '/static/images/5-day-split/day-5/chest-supported-db-row.png',
        length: '3 sets x 8-12 reps',
        href: 'https://www.muscleandstrength.com/exercises/chest-supported-dumbbell-row'
      },
      {
        title: 'Dumbbell Pinwheel curl',
        src: '/static/images/5-day-split/day-5/db-pinwheel-curl.png',
        length: '2 sets x 8-12 reps',
        href:
          'https://www.muscleandstrength.com/exercises/hammer-curl-across-the-body.html'
      },
      {
        title: 'Overhead Dumbbell Tricep Extension',
        src: '/static/images/5-day-split/day-5/overhead-db-tricep-extension.png',
        length: '3 sets x 8-12 reps',
        href:
          'https://www.muscleandstrength.com/exercises/two-arm-standing-dumbbell-extension.html'
      },
      {
        title: 'Dumbbell Shrug',
        src: '/static/images/5-day-split/day-5/db-shrug.png',
        length: '3 sets x 12-15 reps',
        href: 'https://www.muscleandstrength.com/exercises/dumbbell-shrugs.html'
      }
    ]
  }
]
